<?php
    ...
    $location_sql = "SELECT * FROM `catalog_location` WHERE `cat_id`=$catalog_id";
    $location_result = mysqli_query($conn, $location_sql);
    $location_row = mysqli_fetch_assoc($location_result);
    $location_id = $location_row['location_index'];
...
<?php
    ...
    $error_flag = false;
    // Determine if time error & display alert message and turn field(s) pink
    if(isset($_GET['time_error'])){
        $time_error = $_GET['time_error'];

        // 1 - start time error
        // 2 - end time error
        // 3 - start and end time error
        ...
        <section class="catalog_create">
            ...
            <div class="address_area">
                <div class="add_button">
                    ...
                    <input type="hidden" name="highest_location_id" id="highest_location_id" value="<?php echo $location_id; ?>">
            ...
            <p class="invalid_dates_disclaimer disp_none">You cannot update removal information until catalog start and end date is valid</p>
            ...
            <div class="removal_area">
                <div class="add_button">
                    ...
                    <input type="hidden" name="removal_error" id="removal_error">
...
<script>
    ...
    function save_edit_removal(e) {
        ...
        if(obj.end_date){
            ...
            document.getElementById("removal_end_date_entry_" + num).innerHTML = obj.start_date;
        }
        ...
        var catalog_start_date = dt_time($("#start_date").val());
        var catalog_end_date = dt_time($("#end_date").val());

        var removal_start_date = dt_time($("#edit_removal_start_date").val());
        var removal_end_date = dt_time($("#edit_removal_end_date").val());

        var rule_1 = "removal start date must be AFTER catalog end date";
        var rule_2 = "removal start must be BEFORE removal end date";

        if(removal_start_date < catalog_end_date){
            alert("Removal start date must be after the catalog end date");
            return;
        }
        if(removal_end_date < removal_start_date){
            alert("Removal end date must be after the removal start date");
            return;
        }
        ...
        var x = `[name='${n}']`;
        $(x).find(".location_alert").css("display", "none");
        $(x).find('span.date').show();
        $(x).find('strong').show();
        
        var replace_id = "||" + obj.id + "||";
        var removal_error = $("#removal_error").val();
        var new_error = removal_error.replace(replace_id, "||");
        $("#removal_error").val(new_error);
    ...
    function add_location(e) {
        ...
        var location_id = document.getElementById("highest_location_id").value;
        const id = parseInt(location_id) + 1;

        document.getElementById("highest_location_id").value = id;
    ...
    let bid_increment = "<?php echo $row_catalog['bid_increment']; ?>";
    let times_the_bid = "<?php echo $row_catalog['times_the_bid']; ?>";

    let extended_bidding = "<?php echo $row_catalog['extended_bidding']; ?>";
    let extended_type = "<?php echo $row_catalog['extended_type']; ?>";
    let extended_time_remaining = "<?php echo $row_catalog['extended_time_remaining']; ?>";
    let extended_minutes_increment = "<?php echo $row_catalog['extended_minutes_increment']; ?>";
    let extended_minutes_jump = "<?php echo $row_catalog['extended_minutes_jump']; ?>";

    let highest_location_id;

    if(extended_bidding == "no"){
    ...
    let bid_increment_DOM = document.getElementById("bid_increment");
    let times_the_bid_DOM = document.getElementById("times_the_bid");


    let extended_bidding_DOM = document.getElementById("extended_bidding");
    let extended_type_DOM = document.getElementById("extended_type");
    let extended_time_remaining_DOM = document.getElementById("extended_time_remaining");
    let extended_minutes_increment_DOM = document.getElementById("extended_minutes_increment");
    let extended_minutes_jump_DOM = document.getElementById("extended_minutes_jump");
    ...
    bid_increment_DOM.value = bid_increment;
    times_the_bid_DOM.value = times_the_bid;

    extended_bidding_DOM.value = extended_bidding;
    extended_type_DOM.value = extended_type;
    extended_time_remaining_DOM.value = extended_time_remaining;
    extended_minutes_increment_DOM.value = extended_minutes_increment;
    extended_minutes_jump_DOM.value = extended_minutes_jump;
    ...
    function date_checker(type){
            ...
            if(type == "start"){
                ...
                st_date = $("#end_date").val();
            }


            var this_date = st_date;
            var dte = this_date;
    ...
    // const all_inspections = [];
        ...
        function ajax_edit() {
            ...
            allow_registration_questions = $("#registration_question_switch").is(':checked');
        ...
        function validate_catalog_edit() {
            ...
            highest_location_id = document.getElementById("highest_location_id").value;
            ...
            bid_increment = bid_increment_DOM.value;
            times_the_bid = times_the_bid_DOM.value;
            ...
            extended_bidding = extended_bidding_DOM.value;
            extended_type = extended_type_DOM.value;
            extended_time_remaining = extended_time_remaining_DOM.value;
            extended_minutes_increment = extended_minutes_increment_DOM.value;
            extended_minutes_jump = extended_minutes_jump_DOM.value;
            ...
            inspection_notes = $("#cke_inspection_notes iframe").contents().find("body").html();
            removal_notes = $("#cke_removal_notes iframe").contents().find("body").html();
            ...
            catalog_description = $("#cke_catalog_description iframe").contents().find("body").html();
            if (description == "") {
            ...
            if (bid_increment == "") {
                ...
                bid_increment_DOM.style.backgroundColor = "lightpink";
                bid_increment_DOM.focus();
                return;
            }
            ...
            var rem_error = $("#removal_error").val();

            var check_rem = /\d/.test(rem_error);

            console.log(check_rem);
            if(rem_error != "" && rem_error != "||" && check_rem) {
                console.log("rem_error", rem_error);
                alert("Please correct the removal information");
                return;
            }
            ...
            terms_conditions = $("#cke_terms_conditions iframe").contents().find("body").html();
            if (terms == "") {
...
<script>
    ...
    function check_open_time(current_day) {
        current_day = $("#start_date").val();
        current_day = date_formatter(current_day);
        if (current_day) {
            ...
            if (isToday) {
                ...
                if (start_time) {
                    ...
                    alert("Please define the time that bidding opens");
                    $("#start_time").css("background-color", "lightpink");
                }
            }
            else{
...
<script>
    ...
    var time_error = 0;
    <?php if($time_error == "1" || $time_error == 1) : ?>
    ...
    <?php elseif($time_error == "2" || $time_error == 2) : ?>
    ...
    <?php elseif($time_error == "3" || $time_error == 3) : ?>
        
        alert("The catalog cannot begin before today's date and the end date must be after the start date");
...
function validate_dates(){
    ...
    function updateRemovalErrors(){
        
        var invalid_ids = []; 

        var start_date = $("#start_date").val();
        start_date = dt_time(start_date);

        for(var r = 0; r<all_removals.length; r++){

            var validation = true;

            var curr = all_removals[r];

            var rem_id = curr.id;

            var rem_start_date = curr.start_date;
            var rem_end_date = curr.end_date;


            rem_start_date = dt_time(rem_start_date);
            rem_end_date = dt_time(rem_end_date);

            var today = Date.now();
            if(rem_start_date < today){
                validation = false;
            }
            else if(rem_start_date > rem_end_date){
                validation = false;
            }
            else if(rem_start_date < start_date){
                validation = false;
            }

            if(!validation){
                invalid_ids.push(rem_id);
            }
        }

        var error_list = invalid_ids.join("||");
        error_list = "||" + error_list + "||";
        
        $("#removal_error").val(error_list);

        // Loop through the invalid ids and show error message on removal information
        for(var i =0; i<invalid_ids.length; i++){
            var id = invalid_ids[i];
            var name = "li[name='removal_module_" + id + "']";

            // show alert icon
            $(name).find("span.location_alert").css("display", "flex");

            // change labels
            $(name).find(".information_content").children('strong').eq(0).text("Start / End Date:");
            $(name).find(".information_content").children('span.date').eq(0).text("ERROR: Invalid Date(s)");

            $(name).find(".information_content").children('strong').eq(1).hide();
            $(name).find(".information_content").children('span.date').eq(1).hide()

            
        }
    }

    function dt_time(dte){
        var myDate = dte;
    ...
    function fix_removals(){
        $(".removal_area").css("pointer-events", "");
        
        $(".invalid_dates_disclaimer").css("display", "none");
    }
...
<script>
    $(document).ready(function() {
        ...
        function paint_removals_error(removal_list){
            for(var i= 0; i<removal_list.length; i++){
                var rem = removal_list[i];
        ...
        console.log("all_removals", all_removals);

        // disable removal list until catalog start and end date are valid
        // $(".removal_area").css("pointer-events", "none");
        $(".invalid_dates_disclaimer").css("display", "flex");

        var rule_1 = "removal start date must be AFTER catalog end date";
        var rule_2 = "removal start must be BEFORE removal end date";

        var catalog_start_date = $("#start_date").val();
        var catalog_end_date = $("#end_date").val();

        catalog_start_date = dt_time(catalog_start_date);
        catalog_end_date = dt_time(catalog_end_date);

        console.log("rule1:", rule_1);
        console.log("rule2:", rule_2);
        console.log("current:", catalog_start_date, catalog_end_date);
        ...
        var error_ids = invalid_ids.join("||");
        error_ids = "||" + error_ids + "||";

        for(var l = 0; l<invalid_ids.length; l++) {
            var id = invalid_ids[l];

            var module_id = "removal_module_" + id;
            var module_selector = "[name='" + module_id + "']";

            $(module_selector).find(".location_alert").css("display","flex");

            var first_strong = $(module_selector).find(".information_content").children('strong').eq(0);
            first_strong.text("Start / End Date: ");

            var first_span = $(module_selector).find(".information_content").children('span').eq(0);
            first_span.text("ERROR: Invalid Date(s)");

            var second_strong = $(module_selector).find(".information_content").children('strong').eq(1);
            second_strong.text("");

            var second_span = $(module_selector).find(".information_content").children('span').eq(1);
            second_span.text("");
        }
