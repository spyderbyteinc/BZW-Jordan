<?php
    ...
    while($row = mysqli_fetch_assoc($country_result)){
        ...
        array_push($all_country_codes, $line);
    }

    function country_code_display($code, $all_country_codes){
        foreach($all_country_codes as $line){
            $comps = explode("||", $line);
            $cd = $comps[0];
            $cname = $comps[1];
    ...
    // echo country_code_display("US", $all_country_codes);
    ...
    $end_increment = $row['end_increment'];
    ...
    $buyer_premium = $row['buyer_premium'];
    ...
    for($i = 0; $i < 5; $i++){
        $v = $i + 1;
        $element = "asset_location_" . $v;
        $item = $row[$element];

        if($item != -1){
            array_push($all_locations, $item);
        }
    }
    ...
    for($i = 0; $i < 10; $i++){
        ...
        if($item != -1){
            array_push($all_contacts, $item);
        }
    }
    $contact_count = count($all_contacts);

    // Inspection Information
    $all_inspections = array();
    for($i = 0; $i < 10; $i++){
        $v = $i + 1;
        $element = "inspection_" . $v;
        $item = $row[$element];

        if($item != -1){
            array_push($all_inspections, $item);
        }
    }
    $inspection_count = count($all_inspections);
    ...
    for($i = 0; $i < 10; $i++){
        ...
        if($item != -1){
            array_push($all_removals, $item);
        }
    }
    $removal_count = count($all_removals);
?>
...
<script>
        ...
        <?php
            ...
            function get_display_from_mil($mil, $all_times){
...
<div class="catalog_creation_header">
    ...
    <form action="<?php echo $root; ?>processes/process_catalog_edit.php" name="catalog_edit_form" id="catalog_edit_form" method="post">
        <section class="catalog_create">
            ...
            <div class="catalog_row signup_row">
                <div class="col1">
                    <div class="signup_group">
                        ...
                        <input type="text" class="input_text" name="catalog_name" id="catalog_name" placeholder="Catalog Name" value="<?php echo $catalog_name; ?>">
                    </div>
                </div>
            </div>

            <input type="hidden" name="auction_type_selector" id="auction_type_selector" value="<?php echo $auction_type; ?>">

            <input type="hidden" name="charity_event_selector" id="charity_event_selector" value="<?php echo $charity; ?>">
            ...
            <div class="catalog_row signup_row">
                <div class="col1">
                    <div class="signup_group">
                        ...
                        <textarea class="input_text" name="catalog_description" id="catalog_description" rows="3" placeholder="Catalog Description"><?php echo $catalog_description; ?></textarea>
                    </div>
                </div>
            </div>

            <input type="hidden" name="start_time_selector" id="start_time_selector" value="<?php echo $start_time; ?>">
            <input type="hidden" name="end_time_selector" id="end_time_selector" value="<?php echo $end_time; ?>">
            <input type="hidden" name="end_increment_selector" id="end_increment_selector" value="<?php echo $end_increment; ?>">
            ...
            <div class="catalog_row signup_row">
                <div class="col2">
                    <div class="signup_group">
                        ...
                        <input type="text" class="input_text date_picker" onkeypress="return false"   name="start_date" id="start_date" placeholder="mm/dd/yyyy" value="<?php echo $start_date; ?>">
                    </div>
                </div>
                <div class="col2">
                    <div class="signup_group">
            ...
            <div class="catalog_row signup_row">
                <div class="col3">
                    <div class="signup_group">
                        ...
                        <input type="text" class="input_text date_picker"  onkeypress="return false" name="end_date" id="end_date" placeholder="mm/dd/yyyy" value="<?php echo $end_date; ?>">
                    </div>
                </div>
                <div class="col3">
                    <div class="signup_group">
            ...
            <div class="address_area">
                <div class="add_button">
                    ...
                    <ul id="location_list">
                        ...
                        <?php
                                for($q = 0; $q < $location_count; $q++){
                                    $num = $q + 1;
                                    $loc = $all_locations[$q];
                                    $loc_comps = explode("??||&&", $loc);
                                    
                                    $address1 = $loc_comps[0];
                                    $address2 = $loc_comps[1];
                                    $city = $loc_comps[2];
                                    $state = $loc_comps[3];
                                    $country = $loc_comps[4];
                                    ...
                                    $country_display = country_code_display($country, $all_country_codes);
                                    ?>

                                    <li class="location_item" name="location_module_<?php echo $num; ?>"><span><strong>Address: </strong><span class="address" id="location_address1_entry_<?php echo $num; ?>"><?php echo $address1; ?><input type="hidden" name="location_address1_<?php echo $num; ?>" id="location_address1_<?php echo $num; ?>" value="<?php echo $address1; ?>"></span><strong>Address 2: </strong><span class="address" id="location_address2_entry_<?php echo $num; ?>"><?php echo $address2; ?><input type="hidden" name="location_address2_<?php echo $num; ?>" id="location_address2_<?php echo $num; ?>" value="<?php echo $address2; ?>"></span><strong>City: </strong><span class="city" id="location_city_entry_<?php echo $num; ?>"><?php echo $city; ?><input type="hidden" name="location_city_<?php echo $num; ?>" id="location_city_<?php echo $num; ?>" value="<?php echo $city; ?>"></span><strong>State: </strong><span class="state" id="location_state_entry_<?php echo $num; ?>"><?php echo $state; ?><input type="hidden" name="location_state_<?php echo $num; ?>" id="location_state_<?php echo $num; ?>" value="<?php echo $state; ?>"></span><strong>Country: </strong><span class="country" id="location_country_entry_<?php echo $num; ?>"><?php echo $country_display; ?><input type="hidden" name="location_country_<?php echo $num; ?>" id="location_country_<?php echo $num; ?>" value="<?php echo $country; ?>"></span></span><span class="operations"><i class="fas fa-edit cursor_pointer" onclick="edit_location(<?php echo $num; ?>)"></i><i class="far fa-trash-alt cursor_pointer" onclick="delete_location(<?php echo $num; ?>)"></i></span></li>
            ...
            <div class="catalog_row signup_row house_picker">
                ...
                ?>
                    ...
                    ?>
                    <input type="hidden" name="active_house_accounts" id="active_house_accounts" value="<?php echo $house_list; ?>">
            </div>

            <h4 class="sign_heading"><span>Miscellaneous Details</span></h4>

            <input type="hidden" name="currency_selector" id="currency_selector" value="<?php echo $currency; ?>">
            <input type="hidden" name="timezone_selector" id="timezone_selector" value="<?php echo $timezone; ?>">
            ...
            <div class="catalog_row signup_row">
                ...
                <div class="col3">
                    <div class="signup_group">
                        ...
                        <input class="input_text" placeholder="Buyer Premium (%)" type="number" name="buyer_premium" id="buyer_premium" value="<?php echo $buyer_premium; ?>">
                    </div>
                </div>
            </div>
                ...
                <div class="add_button">
                    ...
                    <ul id="inspection_list">
                        <?php
                            ...
                            for($q = 0; $q < $inspection_count; $q++){
                                $num = $q + 1;
                                $ins = $all_inspections[$q];
                                $ins_comps = explode("??||&&", $ins);

                                $date = $ins_comps[0];
                                $start_time = $ins_comps[1];
                                $end_time = $ins_comps[2];
                                $locat = $ins_comps[3];
                                
                                $loc_value = $locat + 1;

                                $my_location = $all_locations[$locat];

                                $loc_comps = explode("??||&&", $my_location);
                                ...
                                $loc_output = $address1 . $address2 . ", " . $city . ", " . $state . ", " . $country;
                                ?>
                                    <li class="inspection_item" name="inspection_module_<?php echo $num; ?>">
                                        <span class="location_alert">
                                            <i class="fas fa-exclamation-circle"></i>
                                        </span>
                                        <span class="information_content">
                                            <strong>Date: </strong>
                                            <span class="date" id="inspection_date_entry_<?php echo $num; ?>">
                                                <?php echo $date; ?>
                                                <input type="hidden" name="inspection_date_<?php echo $num; ?>" id="inspection_date_<?php echo $num; ?>" value="<?php echo $date; ?>">
                                            </span>
                                            <strong>Start Time: </strong>
                                            <span class="time" id="inspection_start_entry_<?php echo $num; ?>">
                                                <?php echo get_display_from_mil($start_time, $all_times); ?>
                                                <input type="hidden" name="inspection_start_<?php echo $num; ?>" id="inspection_start_<?php echo $num; ?>" value="<?php echo $start_time; ?>">
                                            </span>
                                            <strong>End Time: </strong>
                                            <span class="time" id="inspection_end_entry_<?php echo $num; ?>">
                                                <?php echo get_display_from_mil($end_time, $all_times); ?>
                                                <input type="hidden" name="inspection_end_<?php echo $num; ?>" id="inspection_end_<?php echo $num; ?>" value="<?php echo $end_time; ?>">
                                            </span>
                                            <br>
                                            <strong>Location: </strong>
                                            <span class="time" id="inspection_location_entry_<?php echo $num; ?>">
                                                <span class="location_display">
                                                    <?php echo $loc_output; ?>
                                                </span>
                                                <input type="hidden" class="location_missing" name="inspection_location_<?php echo $num; ?>" id="inspection_location_<?php echo $num; ?>" value="<?php echo $loc_value; ?>">
                                            </span>
                                        </span>
                                        <span class="operations">
                                            <i class="fas fa-edit cursor_pointer" onclick="edit_inspection(<?php echo $num; ?>)"></i>
                                            <i class="far fa-trash-alt cursor_pointer" onclick="delete_inspection(<?php echo $num; ?>)"></i>
                                        </span>
                                    </li>
                                <?php 
                            }
            ...
            <div class="removal_area">
                <div class="add_button">
                    ...
                    <ul id="removal_list">
                        <?php
                            for($q = 0; $q < $removal_count; $q++){
                                $num = $q + 1;
                                $rem = $all_removals[$q];
                                $rem_comps = explode("??||&&", $rem);

                                $date = $rem_comps[0];
                                $start_time = $rem_comps[1];
                                $end_time = $rem_comps[2];
                                $locat = $rem_comps[3];
                                ...
                                $loc_output = $address1 . $address2 . ", " . $city . ", " . $state . ", " . $country;
                                ?>
                        <li class="removal_item" name="removal_module_<?php echo $num; ?>">
                            <span class="location_alert">
                                <i class="fas fa-exclamation-circle"></i>
                            </span>
                            <span class="information_content">
                                <strong>Date: </strong>
                                <span class="date" id="removal_date_entry_<?php echo $num; ?>">
                                    <?php echo $date; ?>
                                    <input type="hidden" name="removal_date_<?php echo $num; ?>" id="removal_date_<?php echo $num; ?>" value="<?php echo $date; ?>">
                                </span>
                                <strong>Start Time: </strong>
                                <span class="time" id="removal_start_entry_<?php echo $num; ?>">
                                    <?php echo get_display_from_mil($start_time, $all_times); ?>
                                    <input type="hidden" name="removal_start_<?php echo $num; ?>" id="removal_start_<?php echo $num; ?>" value="<?php echo $start_time; ?>">
                                </span>
                                <strong>End Time: </strong>
                                <span class="time" id="removal_end_entry_<?php echo $num; ?>">
                                    <?php echo get_display_from_mil($end_time, $all_times); ?>
                                    <input type="hidden" name="removal_end_<?php echo $num; ?>" id="removal_end_<?php echo $num; ?>" value="<?php echo $end_time; ?>">
                                </span>
                                <br>
                                <strong>Location: </strong>
                                <span class="time" id="removal_location_entry_<?php echo $num; ?>">
                                    <span class="location_display">
                                        <?php echo $loc_output; ?>
                                    </span>
                                    <input type="hidden" class="location_missing" name="removal_location_<?php echo $num; ?>" id="removal_location_<?php echo $num; ?>" value="<?php echo $loc_value; ?>">
                                </span>
                            </span>
                            <span class="operations">
                                <i class="fas fa-edit cursor_pointer" onclick="edit_removal(<?php echo $num; ?>)"></i>
                                <i class="far fa-trash-alt cursor_pointer" onclick="delete_removal(<?php echo $num; ?>)"></i>
                            </span>
                        </li>
                            <?php } ?>
                    </ul>
            ...
            <div class="contact_area">
                <div class="add_button">
                    ...
                    <ul id="contact_list">
                        <?php
                            for($q = 0; $q < $contact_count; $q++){
                                $num = $q + 1;
                                $con = $all_contacts[$q];
                                $con_comps = explode("??||&&", $con);
                                ...
                                <li class="contact_item" name="contact_module_<?php echo $num; ?>">
                                    <span class="location_alert">
                                        <i class="fas fa-exclamation-circle"></i>
                                    </span>
                                    <span class="information_content">
                                        <strong>Name: </strong>
                                        <span class="name" id="contact_name_entry_<?php echo $num; ?>">
                                            <?php echo $name; ?>
                                            <input type="hidden" name="contact_name_<?php echo $num; ?>" id="contact_name_<?php echo $num; ?>" value="<?php echo $name; ?>">
                                        </span>
                                        <strong>Phone: </strong>
                                        <span class="name" id="contact_phone_entry_<?php echo $num; ?>">
                                            <?php echo $phone; ?>
                                            <input type="hidden" name="contact_phone_<?php echo $num; ?>" id="contact_phone_<?php echo $num; ?>" value="<?php echo $phone; ?>">
                                        </span>
                                        <br>
                                        <strong>Location: </strong>
                                        <span class="phone" id="contact_location_entry_<?php echo $num; ?>">
                                            <span class="location_display">
                                                <?php echo $loc_output; ?>
                                            </span>
                                            <input type="hidden" class="location_missing" name="contact_location_<?php echo $num; ?>" id="contact_location_<?php echo $num; ?>" value="<?php echo $loc_value; ?>">
                                        </span>
                                        </span>
                                    </span>
                                    <span class="operations">
                                        <i class="fas fa-edit cursor_pointer" onclick="edit_contact(<?php echo $num; ?>)"></i>
                                        <i class="far fa-trash-alt cursor_pointer" onclick="delete_contact(<?php echo $num; ?>)"></i>
                                    </span>
                                </li>
                                <?php
                            }
                    ...
                    <ul id="question_list">
                        ...
                        <?php
                            for($q = 0; $q < $question_count; $q++){
                                ...
                                <li class="question_item" name="question_module_<?php echo $num; ?>"><span class="information_content width_100"><strong>Question: </strong><span class="name" id="question_entry_<?php echo $num; ?>"><?php echo $val; ?><input type="hidden" name="question_<?php echo $num; ?>" id="question_<?php echo $num; ?>" value="<?php echo $val; ?>"></span><span class="operations"><i class="fas fa-edit cursor_pointer" onclick="edit_question(<?php echo $num; ?>)"></i><i class="far fa-trash-alt cursor_pointer" onclick="delete_question(<?php echo $num; ?>)"></i></span></span></li>
            ...
            <div class="catalog_row signup_row">
                <div class="col1">
                    <div class="signup_group">
                        ...
                        <textarea class="input_text" name="terms_conditions" id="terms_conditions" rows="3" placeholder="Terms and Conditions"><?php echo $terms; ?></textarea>
                    </div>
                </div>
            </div>

            <input type="hidden" name="confirm_edit" id="confirm_edit" value="<?php echo $catalog_id; ?>"/>
            
            <div class="catalog_row signup_row">
                <a  type="submit" class="fancy-submit fancy-button bg-gradient1 cursor_pointer" style="margin:auto;" href="javascript: validate_catalog_creation();" id="create_account_submit" disabled><span>Edit Catalog</span></a>
            </div>
        </section>


        <input type="hidden" name="question_num" id="question_num" value="<?php echo $question_count; ?>">
        <input type="hidden" name="contact_num" id="contact_num" value="<?php echo $contact_count; ?>">
        <input type="hidden" name="inspection_num" id="inspection_num" value="<?php echo $inspection_count; ?>">
        <input type="hidden" name="removal_num" id="removal_num" value="<?php echo $removal_count; ?>">
        <input type="hidden" name="location_num" id="location_num" value="<?php echo $location_count; ?>">

        <input type="hidden" name="original_question_num" id="original_question_num" value="<?php echo $question_count; ?>">
        <input type="hidden" name="original_contact_num" id="original_contact_num" value="<?php echo $contact_count; ?>">
        <input type="hidden" name="original_inspection_num" id="original_inspection_num" value="<?php echo $inspection_count; ?>">
        <input type="hidden" name="original_removal_num" id="original_removal_num" value="<?php echo $removal_count; ?>">
        <input type="hidden" name="original_location_num" id="original_location_num" value="<?php echo $location_count; ?>">
    </form>
    ...
    <script>
        ...
        $(document).ready(function(){
            ...
            var auction_type = document.getElementById('auction_type_selector').value;
            document.getElementById('auction_type').value=auction_type;

            var charity = document.getElementById('charity_event_selector').value;
            document.getElementById('charity_event').value=charity;

            var start_time = document.getElementById('start_time_selector').value;
            document.getElementById('start_time').value=start_time;

            var end_time = document.getElementById('end_time_selector').value;
            document.getElementById('end_time').value=end_time;
            
            var end_increment = document.getElementById('end_increment_selector').value;
            document.getElementById('end_increment').value=end_increment;

            var currency = document.getElementById('currency_selector').value;
            document.getElementById('asset_currency').value=currency;

            var timezone = document.getElementById('timezone_selector').value;
            document.getElementById('asset_timezone').value=timezone;


            var question_count = $("#original_question_num").val();
            var contact_count = $("#original_contact_num").val();
            var inspection_count = $("#original_inspection_num").val();
            var removal_count = $("#original_removal_num").val();
            var location_count = $("#original_location_num").val();
            ...
            // Set House Selections
            var house_list = document.getElementById('active_house_accounts').value;
        ...
        // Location_addr
        <?php foreach($all_locations as $cur_loc) : ?>
            <?php $loc_comps = explode("??||&&", $cur_loc);
        ...
        ?>
            location_addr.push("<?php echo $line; ?>");
        <?php endforeach ; ?>
...
<script src="../js/validators/catalog_edit.js"></script>